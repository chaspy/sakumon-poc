...
export function MathText({ text, className, style }) {
    ...
    useEffect(() => {
        ...
        // すべてのマッチを位置順にソート
        const allMatches = [
            ...displayMatches.map(m => ({ match: m, type: 'displayMath' })),
            ...inlineMatches.map(m => ({ match: m, type: 'math' }))
        ].sort((a, b) => (a.match.index || 0) - (b.match.index || 0));
        // 重複を除去（display mathが優先）
        const filteredMatches = allMatches.filter((m, i) => {
            if (m.type === 'displayMath')
                return true;
 ...
 */
export function RawMathText({ text, className, style }) {
    return _jsx("div", { className: className, style: style, children: text });
}
